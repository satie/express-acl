...
function config(config, response) {
  ...
  if (config && config.rules) {
    ...
    options.policies = options.rulesObject ? 
                      mapPolicyToGroup(readConfigFile(filePath)[options.rulesObject]) : 
                      mapPolicyToGroup(readConfigFile(filePath));
  }
...
function authorize(req, res, next) {
  const roles = findRoleFromRequest(
    req,
  ...
  let policy;
  roles.forEach(role => {
    policy = options.policies.get(role);
  });
